...
const Home: React.FC<HomeProps> = ({ setSelectedTab }) => {
    const images = [Banner, "https://via.placeholder.com/600x400/3361FF/FFFFFF"];
    ...
    const [shopPromotionList, setShopPromotionList] = useState([]);
    ...
    const mys = [{ selectedSvg: A }, { selectedSvg: B }, { selectedSvg: B }]; // 分类按钮数组
    ...
    const fetchShopPromotionList = async () => {
        try {
            const response = await axios.post("http://kocu.cswallet.co/api/shop/promotion/list");
            const shopPromotionList = response.data; // Assuming the response data is the shop promotion list
            // Handle the shop promotion list data as per your requirements
            setShopPromotionList(shopPromotionList);
        } catch (error) {
            ...
            >
                <Toolbar>
                    ...
                    <Button onClick={handleScanClick} color="inherit" sx={{ position: "relative", left: "25px" }}>
                        <img src={QRcodesvg} alt="HeaderRight" className="h-14"></img>
                    </Button>
                </Toolbar>
            </AppBar>
            <Box
                ...
                <div>
                    ...
                    <div>
                        <Grid container>
                            <Grid item xs={12} sx={{ display: "flex", justifyContent: "center" }}>
                                <div
                                    className="border-gray-100 border shadow-md rounded-[15px] w-[95%] Sample"
                                    style={{
                    ...
                    <div className="mt-5">
                        ...
                        <Grid container justifyContent="center">
                            <Grid item xs={12} className="news" sx={{ position: "relative" }}>
                                <Card>
                                    {/* 左滑动按钮 */}
                                    <IconButton
                                        sx={{
