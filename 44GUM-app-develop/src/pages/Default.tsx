            ...
            <div className="absolute inset-x-0 bottom-0 default-content h-[30%]">
                <div className="flex flex-col items-center default-item">
                    <label className="flex ">
                        <input type="checkbox" name="{isChecked}" value="agree" onChange={handleCheckboxChange} className="mr-1 default-checkbox" />
                        同意相關
                        <p className="text-sky-500 hover:text-sky-700" onClick={handleTCClick}>
                            电子钱包服务使用条款协议
                        </p>
                        及
                        <p className="text-sky-500 hover:text-sky-700" onClick={handleTCClick}>
                ...
                <div className="flex flex-col items-center default-btns">
                    <button onClick={login} className="bg-pink-400 hover:bg-pink-700  default-btn  text-white rounded-full mb-3">
                        {t("login")}
                    </button>
                    <button onClick={register} className="bg-gray-400 hover:bg-gray-700  default-btn  text-white rounded-full">
                        注册
                    </button>
                </div>
                <div className="flex flex-col items-center mt-8 text-gray-700 text-xs">Version: {version}</div>
            </div>
        </div>
    );
}
