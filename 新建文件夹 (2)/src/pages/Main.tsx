        ...
        {showNavbar && (
          <div className='tab-container'>
            <div className="tab fixed bottom-0 w-full bg-white flex border-t border-gray-100 justify-between items-center z-30">
              <Link to="/main/home" onClick={() => { setSelectedTab('home'); setCurrentPage(0); }} className="focus:outline-none tab_icon">
                <img src={getIcon('home')} alt="Home" className='w-full h-full' />
              </Link>
              <Link to="/main/cart" onClick={() => { setSelectedTab('cart'); setCurrentPage(1); }} className="focus:outline-none tab_icon tab_cart">
                <img src={getIcon('cart')} alt="Cart" className='w-full h-full' />
              </Link>
              <div onClick={handleQRCodeClick} className="focus:outline-none flex relative tab_code"> {/* 点击时显示 QR code 页面 */}
                <img src={QRcode} alt="QR Code" className='w-full h-full' />
              </div>
              <Link to="/main/wallet" onClick={() => { setSelectedTab('wallet'); setCurrentPage(2); }} className="focus:outline-none tab_icon tab_wallet">
                <img src={getIcon('wallet')} alt="Wallet" className='w-full h-full' />
              </Link>
              <Link to="/main/user" onClick={() => { setSelectedTab('user'); setCurrentPage(3); }} className="focus:outline-none tab_icon">
